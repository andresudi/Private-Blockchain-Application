...
class Blockchain {
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
            let result = self.chain.filter((data) => data.hash === hash)[0]
            if (result) {
                resolve(result)
            } else {
                resolve(null)
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                for (let data of self.chain) {
                    let decodedData = await data.getBData()
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            for (let block of self.chain) {
                ...
                if (!validateBlock) {
                    errorLog.push({ errMsg: 'Block is invalid!' })
                }

                if (block.previousBlockHash && block.previousBlockHash !== self.chain[block.height - 1].hash) {
                    errorLog.push({ errMsg: 'hash not match with previousBlockHash!' })
                }
            }
            resolve(errorLog)
        });
